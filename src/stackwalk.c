...
static jl_value_t *array_code_info_type = NULL;
...
JL_DLLEXPORT void jl_get_backtrace(jl_array_t **btout, jl_array_t **bt2out)
{
    ...
    if (array_code_info_type == NULL) {
        array_code_info_type = jl_apply_type2((jl_value_t*)jl_array_type, (jl_value_t*)jl_code_info_type, jl_box_long(1));
    }
    bt = jl_alloc_array_1d(array_ptr_void_type, ptls->bt_size);
    memcpy(bt->data, ptls->bt_data, ptls->bt_size * sizeof(void*));
    bt2 = jl_alloc_array_1d(array_ptr_void_type, 0);
